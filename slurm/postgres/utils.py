...
def update_record_status(engine, table, met, logger=None):
    ...
    try:
      ...
      if record:
        ...
        if logger:
            logger.error('Failed to upload status to PostgresQL: %s' % (sys.exc_info()[0]))  


    return record

def update_record_metrics(engine, table, met, logger=None):
    ...
    try:
      ...
      if record:
          ...
          record.elapsed                     = met.elapsed
          record.systime                     = met.systime
          record.usertime                    = met.usertime
          record.walltime                    = met.walltime
          record.percent_of_cpu              = met.percent_of_cpu
          record.maximum_resident_set_size   = met.maximum_resident_set_size
          record.status                      = met.status
        ...
        if logger:
            logger.error('Failed to upload metrics to PostgresQL: %s' % (sys.exc_info()[0]))  
...
def add_pipeline_status(engine, project, uuid, tumor_bam_uuid, case_id,
    ...
    record = update_record_status(engine, statusclass, met, logger)
...
def add_pipeline_metrics(engine, uuid, case_id, download_time,
    ...
    met = metricsclass(uuid                        = uuid,
                       ...
                       elapsed                     = whole_workflow_elapsed,
                       systime                     = main_cwl_systime,
                       usertime                    = main_cwl_usertime,
                       walltime                    = main_cwl_walltime,
                       percent_of_cpu              = main_cwl_percent_of_cpu,
                       maximum_resident_set_size   = main_cwl_maximum_resident_set_size,
                       status                      = status)
